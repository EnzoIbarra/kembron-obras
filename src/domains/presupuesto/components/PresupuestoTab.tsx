...
// ── Three-value cell set — mobile: labeled grid / desktop: grid columns ───────
function AmountCells({
  ...
  return (
    <>
      {/* Mobile: 3-column labeled grid, hidden on md+ */}
      <div className={`mt-2 grid grid-cols-3 gap-1 text-xs md:hidden ${cls}`}>
        <div>
          <div className="text-gray-400 mb-0.5 font-normal">Teórico</div>
          <div>{formatCurrency(theoretical)}</div>
        </div>
        <div>
          <div className="text-gray-400 mb-0.5 font-normal">Real</div>
          <div>{formatCurrency(real)}</div>
        </div>
        <div>
          <div className="text-gray-400 mb-0.5 font-normal">Ejecutado</div>
          <div>{formatCurrency(executed)}</div>
        </div>
      </div>
      {/* Desktop: three separate grid cells */}
      <div className={`hidden md:flex md:items-center md:justify-end text-sm px-4 ${cls}`}>
    ...
    <div className="rounded-xl border border-gray-200 overflow-hidden">
      ...
      {titulo.items.length === 0 ? (
        ...
        titulo.items.map((item) => (
          <div
            key={item.id}
            className="border-b border-gray-100 last:border-b-0 md:grid md:grid-cols-[1fr_140px_140px_140px]"
          >
            {/* Name + unit (always visible) */}
            <div className="px-4 py-3">
              ...
              <AmountCells
                theoretical={item.theoreticalAmount}
                real={item.real}
                executed={item.executed}
              />
            </div>
            {/* Desktop-only amount cells (grid siblings of the name div) */}
            <div className="hidden md:flex md:items-center md:justify-end px-4 text-sm text-gray-700">
              {formatCurrency(item.theoreticalAmount)}
            </div>
            <div className="hidden md:flex md:items-center md:justify-end px-4 text-sm text-gray-700">
      ...
      <div className="bg-gray-50 border-t border-gray-200 md:grid md:grid-cols-[1fr_140px_140px_140px]">
        <div className="px-4 py-3">
          ...
          <AmountCells
            theoretical={titulo.subtotal.theoretical}
            real={titulo.subtotal.real}
            executed={titulo.subtotal.executed}
            bold
          />
        </div>
        <div className="hidden md:flex md:items-center md:justify-end px-4 py-3 text-sm font-semibold text-gray-700">
          {formatCurrency(titulo.subtotal.theoretical)}
        </div>
        <div className="hidden md:flex md:items-center md:justify-end px-4 py-3 text-sm font-semibold text-gray-700">
          {formatCurrency(titulo.subtotal.real)}
        </div>
        <div className="hidden md:flex md:items-center md:justify-end px-4 py-3 text-sm font-semibold text-gray-700">
          {formatCurrency(titulo.subtotal.executed)}
        </div>
      </div>
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col gap-3">
        {[1, 2].map((n) => (
          <div key={n} className="h-32 rounded-xl border border-gray-200 bg-gray-50 animate-pulse" />
        ))}
      </div>
    );
  }
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-gray-500">
          {data?.titulos.length ?? 0} título{data?.titulos.length !== 1 ? 's' : ''}
        </p>
        <button
      ...
      <div className="flex flex-col gap-4">
        {data?.titulos.map((titulo) => (
          <TituloBlock key={titulo.id} titulo={titulo} obraId={obraId} />
        ))}
      </div>
        ...
        <div className="mt-4 rounded-xl border border-gray-300 bg-gray-900 text-white md:grid md:grid-cols-[1fr_140px_140px_140px]">
          <div className="px-4 py-3">
            ...
            <div className="mt-2 grid grid-cols-3 gap-1 text-xs md:hidden font-semibold">
              <div>
                <div className="text-gray-400 mb-0.5 font-normal">Teórico</div>
                <div>{formatCurrency(data.total.theoretical)}</div>
              </div>
              <div>
                <div className="text-gray-400 mb-0.5 font-normal">Real</div>
                <div>{formatCurrency(data.total.real)}</div>
              </div>
              <div>
                <div className="text-gray-400 mb-0.5 font-normal">Ejecutado</div>
                <div>{formatCurrency(data.total.executed)}</div>
              </div>
            </div>
          </div>
          <div className="hidden md:flex md:items-center md:justify-end px-4 py-3 text-sm font-bold">
            {formatCurrency(data.total.theoretical)}
          </div>
          <div className="hidden md:flex md:items-center md:justify-end px-4 py-3 text-sm font-bold">
            {formatCurrency(data.total.real)}
          </div>
          <div className="hidden md:flex md:items-center md:justify-end px-4 py-3 text-sm font-bold">
            {formatCurrency(data.total.executed)}
          </div>
        </div>
      )}
